...
async function tables(env){const q=[`CREATE TABLE IF NOT EXISTS v21_settings(id TEXT PRIMARY KEY,admin_fee INTEGER DEFAULT 0,game_fee INTEGER DEFAULT 0,bank_account TEXT DEFAULT '',access_code TEXT DEFAULT 'TIP2026',rules_html TEXT,competition_name TEXT DEFAULT 'FIFA World Cup 2026',competition_subtitle TEXT DEFAULT 'MS ve fotbale 2026',competition_logo_url TEXT DEFAULT '/worldcup2026-logo.png')`,`CREATE TABLE IF NOT EXISTS v21_users(id TEXT PRIMARY KEY,first TEXT NOT NULL,last TEXT NOT NULL,nick TEXT NOT NULL,email TEXT,phone TEXT,payment_note TEXT,name_norm TEXT NOT NULL UNIQUE,password_hash TEXT NOT NULL,role TEXT DEFAULT 'player',approved INTEGER DEFAULT 0,paid INTEGER DEFAULT 0,points INTEGER DEFAULT 0,tips_count INTEGER DEFAULT 0,evaluated_count INTEGER DEFAULT 0,hit_1 INTEGER DEFAULT 0,hit_3 INTEGER DEFAULT 0)`,`CREATE TABLE IF NOT EXISTS v21_sessions(token TEXT PRIMARY KEY,user_id TEXT NOT NULL,created_at TEXT NOT NULL)`,`CREATE TABLE IF NOT EXISTS v21_seasons(id TEXT PRIMARY KEY,name TEXT,active INTEGER DEFAULT 0)`,`CREATE TABLE IF NOT EXISTS v21_matches(id TEXT PRIMARY KEY,season_id TEXT,phase TEXT,start_time TEXT,home TEXT,away TEXT,home_flag TEXT,away_flag TEXT,status TEXT DEFAULT 'open',score TEXT,confirmed INTEGER DEFAULT 0,info TEXT)`,`CREATE UNIQUE INDEX IF NOT EXISTS v21_match_unique ON v21_matches(season_id,start_time,home,away)`,`CREATE TABLE IF NOT EXISTS v21_bets(user_id TEXT,match_id TEXT,pick TEXT,home_goals INTEGER,away_goals INTEGER,created_at TEXT,PRIMARY KEY(user_id,match_id))`,`CREATE TABLE IF NOT EXISTS v21_groups(id TEXT PRIMARY KEY,name TEXT NOT NULL,code TEXT NOT NULL UNIQUE,admin_fee INTEGER DEFAULT 0,game_fee INTEGER DEFAULT 0,bank_account TEXT DEFAULT '',rules_html TEXT,active INTEGER DEFAULT 1,created_at TEXT DEFAULT CURRENT_TIMESTAMP)`];for(const x of q)await env.DB.prepare(x).run();for(const x of ["ALTER TABLE v21_users ADD COLUMN group_id TEXT DEFAULT 'g-test'","ALTER TABLE v21_settings ADD COLUMN competition_name TEXT DEFAULT 'FIFA World Cup 2026'","ALTER TABLE v21_settings ADD COLUMN competition_subtitle TEXT DEFAULT 'MS ve fotbale 2026'","ALTER TABLE v21_settings ADD COLUMN competition_logo_url TEXT DEFAULT '/worldcup2026-logo.png'"]){try{await env.DB.prepare(x).run()}catch(e){}}}
async function base(env){await env.DB.prepare("INSERT OR IGNORE INTO v21_groups(id,name,code,admin_fee,game_fee,bank_account,rules_html,active) VALUES('g-test','Pokus / testovací','TEST',0,0,'',?,1)").bind(DEFAULT_RULES).run();await env.DB.prepare("UPDATE v21_users SET group_id='g-test' WHERE group_id IS NULL OR group_id=''").run();await env.DB.prepare("INSERT OR IGNORE INTO v21_settings(id,access_code,rules_html,competition_name,competition_subtitle,competition_logo_url) VALUES('main','TIP2026',?,'FIFA World Cup 2026','MS ve fotbale 2026','/worldcup2026-logo.png')").bind(DEFAULT_RULES).run();await env.DB.prepare("UPDATE v21_settings SET rules_html=? WHERE id='main' AND (rules_html IS NULL OR trim(rules_html)='' OR length(rules_html)<800 OR rules_html NOT LIKE '%Skupiny soutěže%')").bind(DEFAULT_RULES).run();await env.DB.prepare("INSERT OR IGNORE INTO v21_seasons(id,name,active) VALUES('football-2026','MS ve fotbale 2026',1)").run();await env.DB.prepare("INSERT OR REPLACE INTO v21_users(id,first,last,nick,email,phone,payment_note,name_norm,password_hash,role,approved,paid,group_id) VALUES('u-admin','Admin','','Admin','','','ADMIN','admin',?,'superadmin',1,1,'g-test')").bind(await hash("admiN9")).run();await ensureOfficialSchedule(env,{clean:false,update:false})}

async function publicGroups(env){await tables(env);await base(env);let groups=(await env.DB.prepare("SELECT id,name,code FROM v21_groups WHERE active=1 ORDER BY name").all()).results||[];return{groups}}
async function resolveGroupId(env,groupValue='',fallbackTest=false){let v=String(groupValue||'').trim();if(!v&&fallbackTest)return 'g-test';if(!v)return '';let n=norm(v);let g=await env.DB.prepare("SELECT * FROM v21_groups WHERE id=? OR lower(code)=? OR lower(name)=? LIMIT 1").bind(v,v.toLowerCase(),n).first();if(g)return g.id;if(fallbackTest)return 'g-test';return ''}
async function createGroup(env,me,b){adm(me);let name=String(b.name||'').trim();if(!name)throw Error('Zadej název skupiny');let code=String(b.code||name).normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9]/g,'').toUpperCase().slice(0,14)||('SKUPINA'+Math.floor(Math.random()*9999));let gid=id('g');await env.DB.prepare("INSERT INTO v21_groups(id,name,code,admin_fee,game_fee,bank_account,rules_html,active) VALUES(?,?,?,?,?,?,?,1)").bind(gid,name,code,+b.admin_fee||0,+b.game_fee||0,b.bank_account||'',b.rules_html||DEFAULT_RULES).run();return{ok:true,id:gid,code}}
async function saveGroup(env,me,b){adm(me);let gid=await resolveGroupId(env,b.id||b.group_id||'',false);if(!gid)throw Error('Skupina nenalezena');await env.DB.prepare("UPDATE v21_groups SET name=?,code=?,admin_fee=?,game_fee=?,bank_account=?,rules_html=? WHERE id=?").bind(b.name||'Skupina',String(b.code||'').toUpperCase(),+b.admin_fee||0,+b.game_fee||0,b.bank_account||'',b.rules_html||DEFAULT_RULES,gid).run();return{ok:true}}
...
return out}
async function findLoginUser(env,raw,groupCode=''){
  raw=String(raw||'').trim();
  let n=norm(raw);
  let nNoSpace=n.replace(/\s+/g,'');
  let email=raw.toLowerCase();
  let groupId=await resolveGroupId(env,groupCode,false);
  if(['admin','admin admin','administrator'].includes(n)){
    await env.DB.prepare("INSERT OR REPLACE INTO v21_users(id,first,last,nick,email,phone,payment_note,name_norm,password_hash,role,approved,paid,group_id) VALUES('u-admin','Admin','','Admin','','','ADMIN','admin',?,'superadmin',1,1,'g-test')").bind(await hash('admiN9')).run();
  }
  let direct=await env.DB.prepare("SELECT * FROM v21_users WHERE (name_norm=? OR lower(email)=? OR lower(nick)=?) AND (?='' OR group_id=? OR role IN ('admin','superadmin')) LIMIT 1").bind(n,email,email,groupId,groupId).first();
  ...
  let rows=(await env.DB.prepare("SELECT * FROM v21_users WHERE (?='' OR group_id=? OR role IN ('admin','superadmin'))").bind(groupId,groupId).all()).results||[];
  return rows.find(u=>{
...
async function login(env,b){
  ...
  if((rawNorm==='admin'||rawNorm==='administrator'||rawNorm==='admin admin') && pass==='admiN9'){
    ...
    await env.DB.prepare("INSERT OR REPLACE INTO v21_users(id,first,last,nick,email,phone,payment_note,name_norm,password_hash,role,approved,paid,group_id) VALUES('u-admin','Admin','','Admin','','','ADMIN','admin',?,'superadmin',1,1,'g-test')").bind(hp).run();
    let tok=id('tok');
    await env.DB.prepare("INSERT INTO v21_sessions VALUES(?,?,datetime('now'))").bind(tok,'u-admin').run();
    return{token:tok,user:'admin'}
  }

  let u=await findLoginUser(env,raw,b.group_id||b.group_code||b.group||'');
...
async function reg(env,b){let first=String(b.first||"").trim(),last=String(b.last||"").trim(),nick=String(b.nick||"").trim(),email=String(b.email||"").trim(),phone=String(b.phone||"").trim(),pass=b.password||"",code=String(b.code||"").trim();if(!first||!last||!nick||pass.length<4)throw Error("Vyplň jméno, příjmení, přezdívku a heslo alespoň 4 znaky");let set=await env.DB.prepare("SELECT * FROM v21_settings WHERE id='main'").first();if(code!==set.access_code)throw Error("Špatný vstupní kód");let groupId=await resolveGroupId(env,b.group_id||b.group_code||b.group||'',true);let n=norm(first+" "+last);if(await env.DB.prepare("SELECT id FROM v21_users WHERE name_norm=? AND group_id=?").bind(n,groupId).first())throw Error("Toto jméno už v této skupině existuje");let uid=id("u"),tok=id("tok"),pn='TIP - '+first+' '+last;await env.DB.prepare("INSERT INTO v21_users(id,first,last,nick,email,phone,payment_note,name_norm,password_hash,role,approved,paid,group_id) VALUES(?,?,?,?,?,?,?,?,?,'player',0,0,?)").bind(uid,first,last,nick,email,phone,pn,n,await hash(pass),groupId).run();await env.DB.prepare("INSERT INTO v21_sessions VALUES(?,?,datetime('now'))").bind(tok,uid).run();return{token:tok}}
...
async function app(env,me,request){await recalc(env);me=await env.DB.prepare("SELECT * FROM v21_users WHERE id=?").bind(me.id).first();let globalSet=await env.DB.prepare("SELECT * FROM v21_settings WHERE id='main'").first();let groups=(await env.DB.prepare("SELECT * FROM v21_groups WHERE active=1 ORDER BY name").all()).results||[];let wanted=new URL(request.url).searchParams.get('group')||me.group_id||'g-test';let group=groups.find(g=>g.id===wanted||g.code===wanted)||groups[0]||{id:'g-test',name:'Pokus / testovací',code:'TEST'};let set={...globalSet,admin_fee:group.admin_fee??globalSet.admin_fee,game_fee:group.game_fee??globalSet.game_fee,bank_account:group.bank_account||globalSet.bank_account,rules_html:group.rules_html||globalSet.rules_html,current_group_id:group.id,current_group_name:group.name,current_group_code:group.code};let sea=await env.DB.prepare("SELECT * FROM v21_seasons WHERE active=1 LIMIT 1").first();let matches=(await env.DB.prepare("SELECT *, CASE WHEN status='finished' THEN 'finished' WHEN status='locked' THEN 'locked' WHEN datetime(start_time) <= datetime('now') THEN 'locked' ELSE status END AS effective_status FROM v21_matches WHERE season_id=? ORDER BY CASE WHEN phase LIKE 'Skupina%' THEN 0 ELSE 1 END, start_time").bind(sea.id).all()).results||[];let bets=(await env.DB.prepare("SELECT * FROM v21_bets WHERE user_id=?").bind(me.id).all()).results||[];let ranking=(await env.DB.prepare("SELECT * FROM v21_users WHERE role='player' AND approved=1 AND paid=1 AND group_id=? ORDER BY points DESC,hit_3 DESC,hit_1 DESC,nick").bind(group.id).all()).results||[];let users=["admin","superadmin"].includes(me.role)?((await env.DB.prepare("SELECT * FROM v21_users WHERE group_id=? OR role IN ('admin','superadmin') ORDER BY role DESC,first,last").bind(group.id).all()).results||[]):[];let publicBets=(await env.DB.prepare("SELECT v21_bets.match_id,v21_bets.home_goals,v21_bets.away_goals,v21_bets.pick,v21_users.nick,v21_users.nick AS public_name FROM v21_bets JOIN v21_users ON v21_users.id=v21_bets.user_id JOIN v21_matches ON v21_matches.id=v21_bets.match_id WHERE v21_users.group_id=? AND (v21_matches.status='finished' OR v21_matches.status='locked' OR datetime(v21_matches.start_time) <= datetime('now')) ORDER BY v21_users.nick").bind(group.id).all()).results||[];let active=ranking.filter(u=>u.approved&&u.paid).length;return{settings:set,groups,currentGroup:group,season:sea,matches,bets,publicBets,ranking:ranking.map(pub),users:users.map(pub),meStats:pub(me),teamOptions:[...new Set([...Object.keys(TEAMS),...matches.flatMap(m=>[m.home,m.away])])].sort(),teamFlags:TEAMS,pool:{players:active,game:active*(set.game_fee||0),admin:active*(set.admin_fee||0)}}}
...
async function settings(env,me,b){adm(me);let gid=await resolveGroupId(env,b.group_id||'',false);await env.DB.prepare("UPDATE v21_settings SET access_code=?,competition_name=?,competition_subtitle=?,competition_logo_url=? WHERE id='main'").bind(b.access_code||'TIP2026',b.competition_name||'FIFA World Cup 2026',b.competition_subtitle||'MS ve fotbale 2026',b.competition_logo_url||'/worldcup2026-logo.png').run();if(gid){await env.DB.prepare("UPDATE v21_groups SET admin_fee=?,game_fee=?,bank_account=?,rules_html=? WHERE id=?").bind(+b.admin_fee||0,+b.game_fee||0,b.bank_account||'',b.rules_html||DEFAULT_RULES,gid).run()}else{await env.DB.prepare("UPDATE v21_settings SET admin_fee=?,game_fee=?,bank_account=?,rules_html=? WHERE id='main'").bind(+b.admin_fee||0,+b.game_fee||0,b.bank_account||'',b.rules_html||DEFAULT_RULES).run()}return{ok:true}}
...
async function clearResults(env,me){adm(me);await env.DB.prepare("UPDATE v21_matches SET score=NULL,confirmed=0,status='open'").run();await recalc(env);return{ok:true}}
async function clearTestData(env,me){adm(me);await env.DB.prepare("DELETE FROM v21_bets").run();await env.DB.prepare("UPDATE v21_matches SET score=NULL,confirmed=0,status='open'").run();await recalc(env);return{ok:true}}
