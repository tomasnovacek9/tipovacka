...
async function login(env,b){
  ...
  const raw=(name||full||login).trim();
  if(!raw || !pass) throw Error('Vyplň jméno a heslo.');

  // Admin se přihlašuje jen: Jméno Admin, příjmení prázdné, heslo admiN9.
  // Když je v DB starší admin s jiným heslem, base() ho při každém API volání znovu nastaví.
  let u=null;
  const n=norm(raw);
  if(['admin','admin admin','administrator'].includes(n)){
    u=await env.DB.prepare("SELECT * FROM v21_users WHERE id='u-admin'").first();
  }
  if(!u) u=await findLoginUser(env,raw);
  if(!u && full && full!==raw) u=await findLoginUser(env,full);
  if(!u && login && login!==raw) u=await findLoginUser(env,login);
  if(!u) throw Error('Účet nebyl nalezen. Zkontroluj jméno a příjmení.');

  const hp=await hash(pass);
  const hpTrim=await hash(pass.trim());
