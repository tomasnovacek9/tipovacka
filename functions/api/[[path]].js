...
async function login(env,b){
  let raw=String(b.name||'').trim();
  let pass=String(b.password||'');
  let u=await findLoginUser(env,raw);
  if(!u)throw Error("Účet nebyl nalezen. Zkontroluj jméno a příjmení, případně zkus přezdívku nebo e-mail.");
